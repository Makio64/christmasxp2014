...
Class(function OrbPulser(j) {
    ...
    this.update = function(q) {
        ...
        l.y = r + s;
        l.z = r;
        l.multiplyScalar(0.15);
        if (!c) {
            h.object3D.scale.lerp(l, 0.1)
        }
...
Class(function LandingView() {
    ...
    function f() {
        k = g.element;
        k.size("100%");
        k.div.style.background = Device.vendor + "linear-gradient(top, #7d73be 0%,#637eac 100%)";
        n = k.create("container");
        n.size("100%").transformPoint("50%", "50%")
    }
    ...
    function d() {
        ...
        l.html("AN AUDIO VISUAL JOURNEY.<br/>" + (Device.mobile ? "TOUCH TO INTERACT." : "MOVE YOUR MOUSE TO INTERACT."))
    }

    function m() {
        e = g.initClass(LandingSpinner, null);
        g.events.bubble(e, HydraEvents.CLICK);
        n.add(e);
        if (!Device.graphics.webgl) {
            l.html("SORRY, WEBGL IS REQUIRED FOR THIS EXPERIENCE");
            e.element.hide()
        }
    }
