class MainScene extends Scene
	...
	createBackground:()=>
		...
		# 	geometry.attributes.position.array[i+2] += 500*Math.random()
	...
	createGUI:()=>
		...
		@gui.add(@,'hitboxVisible').onChange((e)=>
			for i in [0...24] by 1
		...
		@globalOpacity = 1
		@gui.add(@,'globalOpacity',0,1).step(0.01).onChange(()=>
			document.getElementById('webgl').style.opacity = @globalOpacity
		)

		@gui.add(@,'offsetX',-30,30).step(0.1)
	...
	showXP:(index)->
		...
		@mirror.material.envMap = textureCube
	...
	update:(dt)->
		...
		for i in [0...@fragments.length] by 1
			t+=747
			@fragments[i].position.y = @hitboxs[i].position.y+Math.sin(t/350)*1.1
			@fragments[i].position.x = @hitboxs[i].position.x+Math.cos(t/450)*.5
